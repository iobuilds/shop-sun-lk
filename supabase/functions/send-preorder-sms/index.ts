...
serve(async (req) => {
  ...
  try {
    ...
    // Duplicate-send guard (skip for rejected/re-uploadable events)
    ...
    const grandTotal = order.grand_total || ((order.unit_cost_total || 0) + (order.shipping_fee || 0) + (order.tax_amount || 0));
