...
serve(async (req) => {
  ...
  try {
    ...
    const skipDuplicateCheck = ["preorder_payment_rejected", "preorder_arrival_rejected"].includes(template_key);
    ...
    // Fetch pre-order + customer
    const { data: order, error: orderError } = await supabaseAdmin
      .from("preorder_requests").select("*").eq("id", order_id).single();
    if (orderError || !order) throw new Error("Pre-order not found");
    ...
    const grandTotal = order.grand_total || (unitCost + shippingFee + taxAmount);
    ...
    if (phone) {
      ...
      if (smsTemplate) {
        let msg = smsTemplate.message_template
          ...
          .replace(/{{arrival_total}}/g, arrivalTotal.toLocaleString())
          .replace(/{{tracking_link}}/g, "")
          .replace(/{{admin_notes}}/g, order.admin_notes ? ` ${order.admin_notes}` : "")
    ...
    console.error("Pre-order SMS error:", error);
    return new Response(
